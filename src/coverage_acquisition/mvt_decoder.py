...
def load_vector_tile_message_class():
    try:
        ...
        raise RuntimeError("mapbox_vector_tile package is required for the vector_tile protobuf schema.") from exc
...
def transform_geometry_to_lonlat(geometry: dict, tile_x: int, tile_y: int, zoom: int, extent: int) -> dict:
    ...
    if geometry_type == "MultiPoint":
        return {
            "type": "MultiPoint",
            "coordinates": [list(tile_point_to_lonlat(tile_x, tile_y, zoom, extent, point)) for point in coords],
        }
    ...
    if geometry_type == "MultiLineString":
        return {
            ...
            "coordinates": [
                [list(tile_point_to_lonlat(tile_x, tile_y, zoom, extent, point)) for point in line] for line in coords
            ],
        }
    ...
    if geometry_type == "Polygon":
        return {
            "type": "Polygon",
            "coordinates": [
                [list(tile_point_to_lonlat(tile_x, tile_y, zoom, extent, point)) for point in ring] for ring in coords
            ],
        }
...
def geometry_to_wkt(geometry: dict) -> str:
    ...
    if geometry_type == "MultiLineString":
        inner = ", ".join("(" + ", ".join(fmt_point(point) for point in line) + ")" for line in coords)
        return f"MULTILINESTRING ({inner})"
    if geometry_type == "Polygon":
        inner = ", ".join("(" + ", ".join(fmt_point(point) for point in ring) + ")" for ring in coords)
...
def feature_rows_from_decoded_tile(
    *,
    ...
    selected_layers = set(include_layers) if include_layers else None
    feature_records = []
    layer_counts = {
        layer_name: layer_payload.get("feature_count", 0)
        for layer_name, layer_payload in decoded_tile.items()
        if selected_layers is None or layer_name in selected_layers
    }
