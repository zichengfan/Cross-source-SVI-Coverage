...
from coverage_acquisition.kakao_frontend import discover_kakao_roadview_frontend_config
...
from coverage_acquisition.naver_frontend import discover_naver_panorama_frontend_config
...
def _fetch_job(provider: ProviderDefinition, source: SourceDefinition, job: dict, request: FetchAreaRequest) -> dict:
    ...
    for x, y in iter_tile_coords(_tile_range_from_dict(job["source_tile_range"])):
        ...
        try:
            ...
            # A bare socket TimeoutError (distinct from urllib's HTTPError/URLError)
            # surfaces when a read genuinely hangs past timeout_seconds -- observed
            # against Mapilio's geo.mapilio.com for tiles with dense geometry that
            # its GeoServer backend can't render in time. Without this, one such
            # tile aborts the whole provider fetch even with stop_on_error=False.
        ...
        try:
            if source.kind == "raster":
                if _is_yandex_stv_source(source) and (http_status == 204 or not payload):
                    is_empty = True
                    stored_payload = b""
                elif source.expect_content_type_prefix and not content_type.startswith(
                    ...
                    if _is_yandex_stv_source(source) or _uses_alpha_coverage(source):
                        is_empty = coverage_pixel_count == 0

                if is_empty is not True:
                    suffix = suffix_for_content_type(content_type, tile_url, default=".png")
                    tile_path = (
                        output_dir / source.effective_storage_subdir / str(job["source_zoom"]) / str(x) / f"{y}{suffix}"
                    )
                ...
                tile_path = (
                    output_dir / source.effective_storage_subdir / str(job["source_zoom"]) / str(x) / f"{y}.json"
                )
                ensure_directory(tile_path.parent)
...
def _build_runtime_options(source: SourceDefinition, request: FetchAreaRequest) -> dict:
    ...
    if config_kind == "naver_panorama":
        return _build_naver_runtime_options(source, request)
    if config_kind == "kakao_roadview":
        return _build_kakao_runtime_options(source, request)
...
def _build_naver_runtime_options(source: SourceDefinition, request: FetchAreaRequest) -> dict:
    fallback_version = source.options.get("version_fallback", "")
    runtime_options = {
        "frontend_config": {
            "config_source": "fallback",
    ...
    try:
        config = discover_naver_panorama_frontend_config(
            user_agent=source.headers.get("User-Agent", "global-svi-coverage-observatory/0.2"),
            timeout_seconds=request.timeout_seconds or 60,
        )
    except Exception as exc:
        runtime_options["frontend_config"]["config_error"] = repr(exc)
    ...
    coverage_tile_template = config.street_only_overlay_template
    runtime_options["tile_template"] = coverage_tile_template
    runtime_options["frontend_config"] = {
        "config_source": "live_style_metadata",
        "version": config.version,
        "coverage_tile_template": coverage_tile_template,
        "overlay_type": "ps",
        "air_water_icons_visible": False,
        "composite_panorama_template": config.panorama_overlay_template,
    }
    return runtime_options


def _build_kakao_runtime_options(source: SourceDefinition, request: FetchAreaRequest) -> dict:
    runtime_options = {"frontend_config": {"config_source": "fallback"}}
    try:
        config = discover_kakao_roadview_frontend_config(
            user_agent=source.headers.get("User-Agent", "global-svi-coverage-observatory/0.2"),
            timeout_seconds=request.timeout_seconds or 60,
        )
    except Exception as exc:
    ...
    runtime_options["tile_template"] = config.roadview_hd_template
    runtime_options["frontend_config"] = {
        "config_source": "live_sdk_loader",
        "sdk_loader_url": config.sdk_loader_url,
        "runtime_bundle_versions": config.runtime_bundle_versions,
        "roadview_hd_template": config.roadview_hd_template,
    }
    return runtime_options
...
def _uses_alpha_coverage(source: SourceDefinition) -> bool:
    return source.options.get("coverage_from") == "alpha" or source.options.get("empty_tile_rule") == "transparent_png"
    ...
    *,
    ...
    if source.vector_decoder == "custom_mvt":
        ...
        return feature_rows_from_decoded_tile(
            ...
            tile_url=tile_url,
            fetched_at=fetched_at,
            include_layers=source.layer_names or None,
        )
